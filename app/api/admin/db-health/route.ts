...
export async function GET(req: Request) {
  ...
  const sampleLimit = Math.max(1, Math.min(50, Math.trunc(Number(url.searchParams.get("sampleLimit") || 12))));
  return Response.json({
    ok: true,
    db: dbIntegrityReport({ sampleLimit }),
    ecs: ecsDbShapeChecklist(),
    worldTick: worldTickStatus(),
