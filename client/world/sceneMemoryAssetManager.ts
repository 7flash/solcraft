...
export type DisposeObjectOptions = { detach?: boolean; disposeGeometry?: boolean; disposeMaterial?: boolean; disposeTextures?: boolean; sharedGeometries?: ReadonlySet<THREE.BufferGeometry>; sharedMaterials?: ReadonlySet<THREE.Material>; sharedTextures?: ReadonlySet<THREE.Texture> };
const TEXTURE_KEYS = ["map", "alphaMap", "aoMap", "bumpMap", "displacementMap", "emissiveMap", "envMap", "lightMap", "metalnessMap", "normalMap", "roughnessMap", "specularMap", "gradientMap"];
export function disposeObject3D(target: THREE.Object3D | null | undefined, options: DisposeObjectOptions = {}) {
  if (!target) return;
  const opts = { detach: options.detach ?? true, disposeGeometry: options.disposeGeometry ?? true, disposeMaterial: options.disposeMaterial ?? true, disposeTextures: options.disposeTextures ?? true, sharedGeometries: options.sharedGeometries, sharedMaterials: options.sharedMaterials, sharedTextures: options.sharedTextures };
