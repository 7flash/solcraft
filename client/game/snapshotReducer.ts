...
export function applySnapshot(state: ClientWorldState, snap: ClientSnapshot): ClientWorldState {
  ...
  if (world) {
    ...
    if (Number.isFinite(Number(world.ax)) && Number.isFinite(Number(world.az))) state.anchor = { x: Number(world.ax), z: Number(world.az) };
    ...
    applyWorldDelta(state, (world.delta || world.changes || world) as ClientWorldDelta);
  }
  if (Array.isArray(snap.players)) for (const p of snap.players) {
...
export function applyWorldDelta(state: ClientWorldState, delta: ClientWorldDelta | null | undefined): ClientWorldState {
  ...
  if (Number.isFinite(Number(delta.ax)) && Number.isFinite(Number(delta.az))) state.anchor = { x: Number(delta.ax), z: Number(delta.az) };
...
function replaceById(items: any[], getId: (item: any) => number): Map<number, any> {
  ...
  for (const item of items) { const id = getId(item); if (id) m.set(id, item); }
  return m;
}
...
function applyCoordDelta(map: Map<string, any>, upsert?: any[], remove?: any[]) {
  if (Array.isArray(remove)) for (const item of remove) { const k = coordRemoveKey(item); if (k) map.delete(k); }
  if (Array.isArray(upsert)) for (const item of upsert) { const k = coordKey(Number(item?.x || 0), Number(item?.z || 0)); map.set(k, { ...(map.get(k) || {}), ...item }); }
}
...
function applyIdDelta(map: Map<number, any>, upsert: any[] | undefined, remove: any[] | undefined, getId: (item: any) => number) {
  if (Array.isArray(remove)) for (const item of remove) { const id = idRemoveKey(item, getId); if (id) map.delete(id); }
  if (Array.isArray(upsert)) for (const item of upsert) { const id = getId(item); if (id) map.set(id, { ...(map.get(id) || {}), ...item }); }
}
export function snapshotSignature(state: ClientWorldState): string {
  return [state.rev, state.me?.id || 0, state.me?.x || 0, state.me?.z || 0, state.tiles.size, state.buildings.size, state.doodads.size, state.loot.size, state.players.size, state.events.length].join(":");
}
