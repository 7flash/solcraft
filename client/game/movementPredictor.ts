...
export type ReconcileResult = { corrected: boolean; lerp: boolean; authoritative: GridCoord; predicted: GridCoord; pending: number; drift: number };
...
export class MovementPredictor {
  ...
  reconcile(serverPos: GridCoord, serverSeq?: number, options: { softTiles?: number; hardTiles?: number } = {}): ReconcileResult {
    ...
    if (d > hardTiles) { this.pending = []; return { corrected: true, lerp: false, authoritative, predicted: authoritative, pending: 0, drift: d }; }
    if (d > softTiles) return { corrected: true, lerp: true, authoritative, predicted, pending: this.pending.length, drift: d };
    return { corrected: false, lerp: false, authoritative, predicted, pending: this.pending.length, drift: d };
  }
