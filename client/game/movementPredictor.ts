...
function cloneCoord(v: GridCoord): GridCoord {
  return { x: Number(v.x || 0), z: Number(v.z || 0) };
}

function dist(a: GridCoord, b: GridCoord) {
...
export class MovementPredictor {
  ...
  pendingMoves(): readonly PredictedMove[] {
    return this.pending;
  }

  getCurrentPredictedPosition(): GridCoord | null {
    if (this.pending.length) return cloneCoord(this.pending[this.pending.length - 1].to);
    return this.lastConfirmedPos ? cloneCoord(this.lastConfirmedPos) : null;
  }
  ...
  reconcile(serverPos: GridCoord, serverSeq?: number, options: { softTiles?: number; hardTiles?: number } = {}): ReconcileResult {
    ...
    if (Number.isFinite(Number(serverSeq))) {
      const ack = Number(serverSeq);
      this.pending = this.pending.filter((m) => m.seq > ack && m.status !== "rejected");
    }
    ...
    if (d > hardTiles) {
      this.pending = [];
      return { corrected: true, lerp: false, authoritative, predicted: authoritative, pending: 0 };
    }

    if (d > softTiles) {
      return { corrected: true, lerp: true, authoritative, predicted, pending: this.pending.length };
    }
