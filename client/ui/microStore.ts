...
export class MicroStore<T extends Record<string, any>> {
  ...
  patch(values: Partial<T>) {
    this.batching++;
    try {
      for (const [k, v] of Object.entries(values)) this.set(k as keyof T, v as T[keyof T]);
    } finally {
      this.batching--;
    }
    if (!this.batching && this.pendingPrev.size) {
      const pending = this.pendingPrev;
      this.pendingPrev = new Map();
      this.flush(pending);
    }
  }
  ...
  subscribe<V>(
    selector: (state: Readonly<T>) => V,
    callback: (value: V, prev: V) => void,
    options: { immediate?: boolean; equals?: Equality<V> } = {},
  ): Unsubscribe {
    const listener: SelectorListener<T, V> = {
      selector,
      callback,
      equals: options.equals || sameValue,
  ...
  bindText<K extends keyof T>(
    key: K,
    el: HTMLElement | null,
    format: (value: T[K]) => string = (v) => String(v ?? ""),
  ): Unsubscribe {
    if (!el) return () => {};
    return this.subscribeKey(key, (value) => {
      const next = format(value);
      if (el.textContent !== next) el.textContent = next;
    });
  }

  bindStyle<K extends keyof T>(
    key: K,
    el: HTMLElement | null,
    ...
    return this.subscribeKey(key, (value) => {
      const next = format(value);
      if ((el.style as any)[prop] !== next) (el.style as any)[prop] = next;
    });
  }

  bindRender<K extends keyof T>(
    key: K,
    renderValue: (value: T[K], prev: T[K]) => void,
  ): Unsubscribe {
    return this.subscribeKey(key, renderValue);
  }

  private flush(prevByKey: Map<keyof T, any>) {
    for (const [key, prev] of prevByKey) {
      const value = this.state[key];
      this.keyListeners.get(key)?.forEach((cb) => cb(value, prev));
    }

    for (const listener of this.selectorListeners) {
      const prev = listener.value;
      const next = listener.selector(this.state);
      if (!listener.equals(prev, next)) {
        listener.value = next;
        listener.callback(next, prev);
      }
    }
  }
}
