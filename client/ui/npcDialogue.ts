...
export function npcTalkLine(p: any) {
  ...
  if (p?.role === "trader") return t("npc.trader", "{name}: Donate supplies to earn reputation, or leave me to gather resources.{carrying}", { name, carrying });
  if (p?.role === "warrior") return t("npc.warrior", "{name}: Keep your blade sheathed unless you want your reputation to suffer.{carrying}", { name, carrying });
